...
func main() {
	var (
		platform     = flag.String("-platform", "local", "Set which cloud platform to use, defaults to local")
		...
		defaultZone  = flag.String("-default-zone", "local", "Set the default zone to spawn  and manage instances, defaults to local")
	...
	log.Println("Registering initial master in", *defaultZone, "zone")
	regBytes, regErr := mongoHandler.Manager.Register(*defaultZone, "master", *instances)
	if reqErr != nil {
		panic(regErr)
	}
